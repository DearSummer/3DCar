...
namespace Geometry {
	...
	namespace Internal
	{
		...
		void InsertVertexElement(VertexType& vertexDst,const VertexData& vertexSrc)
		{
			...
			static const std::map<std::string,std::pair<size, size>> semanticSizeMap =
			{
				{"POSITION",std::pair<size,size>(0,12)},
				{"NORMAL",std::pair<size,size>(12,24)},
				{"TANGENT",std::pair<size,size>(24,40)},
				{"COLOR",std::pair<size,size>(40,56)},
				{"TEXCOORD",std::pair<size,size>(56,64)}
			};

			for(size i = 0;i < ARRAYSIZE(VertexType::inputLayout);i++)
			{
	...
	template<class VectexType = VertexPosNormalTex,class IndexType = DWORD>
	MeshData<VectexType, IndexType> CreateBox(float width = 2.0f, float height = 2.0f, float depth = 2.0f,
...
inline Geometry::MeshData<VertexType, IndexType> Geometry::CreateBox(float width, float height, float depth, const DirectX::XMFLOAT4 & color)
{
	...
	Geometry::MeshData<VertexType, IndexType> meshData;
	meshData.vertexVec.resize(24);


	Geometry::Internal::VertexData vertexDataArr[24];
...
template<class VertexType, class IndexType>
inline Geometry::MeshData<VertexType, IndexType> Geometry::CreateCylinder(float radius, float height, UINT slices,
	const DirectX::XMFLOAT4& color)
{
	...
	for (UINT i = 0; i <= slices; ++i)
	{
		theta = i * per_theta;
		vertexData = { 
			XMFLOAT3(radius * cosf(theta), -h2, radius * sinf(theta)), 
			XMFLOAT3(0.0f, -1.0f, 0.0f),
			XMFLOAT4(-1.0f, 0.0f, 0.0f, 1.0f), 
			color, 
			XMFLOAT2(cosf(theta) / 2 + 0.5f, sinf(theta) / 2 + 0.5f) };
...
// 创建只有圆柱体侧面的网格数据，slices越大，精度越高
template<class VertexType, class IndexType>
Geometry::MeshData<VertexType, IndexType> Geometry::CreateCylinderNoCap(float radius, float height, UINT slices,
	const DirectX::XMFLOAT4& color)
{
	using namespace DirectX;
	...
	for (UINT i = 0; i <= slices; ++i)
	{
		...
		vertexData = { 
			XMFLOAT3(radius * cosf(theta), h2, radius * sinf(theta)), 
			XMFLOAT3(cosf(theta), 0.0f, sinf(theta)),
			XMFLOAT4(-sinf(theta), 0.0f, cosf(theta), 1.0f), 
			color,
...
template<class VertexType, class IndexType>
inline Geometry::MeshData<VertexType, IndexType> Geometry::CreatePlane(const DirectX::XMFLOAT2 & planeSize,
...
template<class VertexType, class IndexType>
inline Geometry::MeshData<VertexType, IndexType> Geometry::CreatePlane(float width, float depth, float texU, float texV, const DirectX::XMFLOAT4 & color)
{
	using namespace DirectX;
	...
	Internal::VertexData vertexData = { XMFLOAT3(-width / 2, 0.0f, -depth / 2), XMFLOAT3(0.0f, 1.0f, 0.0f),
